...
barplot(table(pop(myData_genind_allMarkersOnly)), col=funky(17), las=3,
xlab="Population", ylab="Sample size")

temp <- summary(myData_genind_allMarkersOnly)
temp
barplot(myData_genind_allMarkersOnly$loc.n.all,table(locNames(myData_genind_allMarkersOnly)),col=funky(17),las=3,xlab="",ylab="Number of alleles")
temp$Hexp

# Get the number of alleles for each locus
num_alleles <- myData_genind_allMarkersOnly$loc.n.all
locus_names <- locNames(myData_genind_allMarkersOnly)

# Create a data frame for the number of alleles and locus names
allele_table <- data.frame(Locus = locus_names, Number_of_Alleles = num_alleles)

# Print the table
kable(allele_table)

# Create a barplot
barplot(num_alleles, names.arg = locus_names, col = funky(17), las = 3,
        xlab = "", ylab = "Number of Alleles", main = "Number of Alleles by Locus")
kable(locus_table(myData_genclone_allMarkersOnly))
```
