...
x.pops_cc <- tab(cc_myData_genind_allMarkersOnly,
                    freq=TRUE, NA.method="mean")
pca.pops_cc <- dudi.pca(df = x.pops_cc,
                           center = TRUE, scale = FALSE, scannf = FALSE, nf = 100)

# Calculate explained variance for each PCA axis
cc.explained_variance <- pca.pops_cc$eig / sum(pca.pops_cc$eig) * 100
barplot(cc.explained_variance, main = "Explained Variance per PCA Axis", xlab = "Principal Component", ylab = "Variance Explained (%)")


#looking at first two axes:
s.class(pca.pops_cc$li,
        fac=pop(cc_myData_genind_allMarkersOnly),
        col=transp(funky(15),0.6),axesel=FALSE,cstar=0,cpoint=3)
add.scatter.eig(pca.pops_cc$eig[1:50],3,1,2, ratio=.17)
...
```
```{r PCA new way, eval=FALSE, include=FALSE}
setPop(myData_genind_allMarkersOnly) <- ~Pop
