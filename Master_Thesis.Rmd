...
#read in genalex data, cleaning markers, genetic distance matrix and creating clonecorrected dataset

T_all <- read_excel("Tuaest_Master_genMonit_haploid_MP_Mai2024.xlsx")
GenDist_spagedi <- read_excel("Genetic_distance_FST_ANOVA_Spagedi.xlsx") %>%
  column_to_rownames(var="...1")
GenDist_RST_spagedi <- read_excel("Genetic_distance_Rst_Spagedi.xlsx") %>%
  column_to_rownames(var="...1")
...
myData_genind_allMarkersOnly@other$xy <- coords
...
Structure_ClusterAssignMLGs_k5 <- read_excel("Structure_ClusterAssignMLGs_k5_forPies.xlsx")
Structure_ClusterAssignMLGs_k6 <- read_excel("Structure_ClusterAssignMLGs.xlsx")
...
#emf(file="coordinates_truffle_sites.emf")
ggplot(data = europe) +
  geom_sf(fill = "lightgrey", color = "black") +
  geom_text(data = europe_points, aes(x = X, y = Y, label = name),
            color = "darkmagenta", fontface = "bold", size = 2.5, check_overlap = FALSE) +
  annotation_scale(location = "bl", width_hint = 0.5) +
  coord_sf(xlim = c(5, 11), ylim = c(45.5, 49), expand = FALSE) +
  geom_point(data = coord, aes(x = LON, y = LAT),
             shape = 21, color = "darkcyan", fill = "darkcyan", size = 1.5) +
  geom_text_repel(data = coord, aes(x = LON, y = LAT, label = Site_1_abrev),
                   color = "black",
                   size = 2.5,
                   box.padding = 0.2,
                   point.padding = 0.4,
                   ...
                   max.overlaps = Inf) +
  labs(title = "Map of monitoring sites",
       x = "",
       y = "") +
  theme_minimal() +
...
cc_pca_scores5$Cluster <- factor(cc_pca_scores5$Cluster,levels=c("1","2","3","4","5","mixed"))
...
PCA_k5_colored <- cc_plot_k5_1 + cc_plot_k5_2
ggsave("PCA_k5_colored.emf",width=10, height=4, device = {function(filename, ...) devEMF::emf(file = filename)})
...
pca_scores6$Cluster <- factor(pca_scores6$Cluster,levels=c("1","2","3","4","5","6","mixed"))
...
cc_pca_scores6$Cluster <- factor(cc_pca_scores6$Cluster, levels = c("1","2","3","4","5","6","mixed"))
...
PCA_k6_colored <- cc_plot_k6_1 + cc_plot_k6_2
ggsave("PCA_k6_colored.emf",width=10, height=4, device = {function(filename, ...) devEMF::emf(file = filename)})
...
cc_plot_k6_5 <- ggplot(cc_pca_scores6, aes(x = Axis1, y = Axis2, color = Cluster)) +
  geom_point() +
  gghighlight::gghighlight() + 
  facet_wrap(vars(Cluster)) +
  xlab(paste("PC1 (", round(cc_explained_variance_k6[1], 2), "%)", sep = "")) +
...
print(cc_plot_k6_5 + cc_plot_k6_6)
```
...
spca(myData_genind_allMarkersOnly)
```
...
anc.mat_k5 <- Structure_ClusterAssignMLGs_k5 %>%
  select(7,6,1:5) %>%
  data.frame()
colnames(anc.mat_k5) <-c("Site","Ind","Cluster 1","Cluster 2","Cluster 3","Cluster 4","Cluster 5")

anc.mat_k5_withoutlocprior <- Structure_ClusterAssignMLGs_k5 %>%
  select(7,6,8:12) %>%
  data.frame()
colnames(anc.mat_k5_withoutlocprior) <-c("Site","Ind","Cluster 1","Cluster 2","Cluster 3","Cluster 4","Cluster 5")

color_palette_pies_k5 <- c("#377EB8",
                   "#FF7F00",
                   "navy",
                   "#4DAF4A",
                   "mediumvioletred")
...
(map_k5 <- mapmixture(
  admixture_df = anc.mat_k5,
  coords_df = pops,
  cluster_cols = color_palette_pies_k5,
...
ggsave("map_k5.emf",width=10, height=4, device = {function(filename, ...) devEMF::emf(file = filename)})

(map_k5_withoutLocPrior <- mapmixture(
  admixture_df = anc.mat_k5_withoutlocprior,
  coords_df = pops,
  cluster_cols = color_palette_pies_k5,
  cluster_names = c("Cluster 1","Cluster 2","Cluster 3","Cluster 4", "Cluster 5"),
  crs = 2056,
...
ggsave("map_k5_withoutLocPrior.emf",width=10, height=4, device = {function(filename, ...) devEMF::emf(file = filename)})

#----------------------------------------------------------

anc.mat_k6 <- Structure_ClusterAssignMLGs_k6 %>%
select(4,12,5:10) %>%
  data.frame()
colnames(anc.mat_k6) <-c("Site","Ind","Cluster 1","Cluster 2","Cluster 3","Cluster 4","Cluster 5","Cluster 6")

anc.mat_k6_withLocPrior <- Structure_ClusterAssignMLGs_k6 %>%
select(4,12,29:34) %>%
  data.frame()
colnames(anc.mat_k6_withLocPrior) <-c("Site","Ind","Cluster 1","Cluster 2","Cluster 3","Cluster 4","Cluster 5","Cluster 6")
...
ggsave("map_k6.emf",width=10, height=4, device = {function(filename, ...) devEMF::emf(file = filename)})

(map_k6_withLocPrior <- mapmixture(
  admixture_df = anc.mat_k6_withLocPrior,
  coords_df = pops,
  cluster_cols = color_palette_pies_k6,
  cluster_names = c("Cluster 1","Cluster 2","Cluster 3","Cluster 4", "Cluster 5","Cluster 6"),
  crs = 2056,
...
ggsave("map_k6_withLocPrior.emf",width=10, height=4, device = {function(filename, ...) devEMF::emf(file = filename)})
```
...
#-----------------------------------------------------------------------------
# Table for thesis with Pop, #samples, #Genotypes, #genotypic diversity, # Simpson's

setPop(myData_genind_allMarkersOnly) <- ~Pop
table1
table1 <- poppr(myData_genind_allMarkersOnly)
table1 <- table1 %>%
  select(Pop,N,MLG) %>%
  mutate("Genotypic diversity" = ((MLG-1)/(N-1))) %>%
  arrange(Pop)
table1
  
diversity_ci(myData_genind_allMarkersOnly, n = 100L, raw = FALSE, H=FALSE, G=FALSE)
tab <- mlg.table(myData_genind_allMarkersOnly)
diversity_ci(tab,n=100L, raw = FALSE, H=FALSE, G=FALSE)
```
...
## FST Values
```{r fst values}
Gst_Hedrick(myData_genind_allMarkersOnly)

#https://www.rdocumentation.org/packages/hierfstat/versions/0.5-11/topics/pairwise.neifst

setPop(myData_genind_allMarkersOnly) <- ~Pop
pairwise.neiFst_aMO <- pairwise.neifst(myData_genind_allMarkersOnly, diploid=FALSE)
pairwise.neiFst_aMO
pairwise.Fst_aMO <- pegas::pairwise.fst(myData_genind_allMarkersOnly)
...
#-------------------------------------------------------------------------------------
#https://www.rdocumentation.org/packages/graph4lg/versions/1.6.0/topics/mat_pw_fst
...
#korreliere matFst & die zwei Matrizen um zu sehen ob signifikant, hohes R
...
#-----------------------------------------------------------------------
#try to get pairwise phipt

# Extract unique populations from the genind object
populations <- levels(pop(myData_genind_allMarkersOnly))

# Initialize a list to store results
pairwise_amova_results <- list()

# Nested loops for pairwise comparisons
for (i in 1:(length(populations) - 1)) {
  for (j in (i + 1):length(populations)) {
    
    # Get the current pair of populations
    pop1 <- populations[i]
    pop2 <- populations[j]
    
    # Subset the genind object for the two populations
    subset_genind <- myData_genind_allMarkersOnly[pop(myData_genind_allMarkersOnly) %in% c(pop1, pop2), ]
    
    # Run AMOVA on the subset
    amova_result <- poppr.amova(subset_genind, ~Pop)
    
    # Extract PhiPT (or assign NA if AMOVA fails)
    phipt <- tryCatch(
      amova_result$components$Phi[["Among populations"]],
      error = function(e) NA
    )
    
    # Store results in the list
    pairwise_amova_results[[paste(pop1, pop2, sep = "_vs_")]] <- list(
...
pairwise_phipt <- do.call(rbind, lapply(names(pairwise_amova_results), function(pair) {
  data.frame(
    Population_Pair = pair,
    Pop1 = pairwise_amova_results[[pair]]$populations[1],
    Pop2 = pairwise_amova_results[[pair]]$populations[2],
    PhiPT = pairwise_amova_results[[pair]]$phipt
  )
}))

# View the summarized results
print(pairwise_phipt)
...
genetic_dist <- dist.genpop(myData_genpop_allMarkersOnly, method = "1")  # You can choose other methods if needed
genetic_dist
...
distances_df <- data.frame(
  Geographic_Distance = log(lower_triangle_geo),
  Genetic_Distance = lower_triangle_genetic
)
...
#-----------------------------------------------------------------------
#trying to find the mismatch between Genalex and R

setPop(myData_genind_allMarkersOnly_) <- ~Pop
myData_genpop_allMarkersOnly <- genind2genpop(myData_genind_allMarkersOnly)


#creating log10(1+geodist) --- verified with Genalex Results

coord_minusBAR <- coord[!grepl("BAR",coord$Site_1_abrev),]
coords_minusBAR <- coord_minusBAR[, c("LON", "LAT")]

coordsgeo_dist_minusBAR <- distm(coords_minusBAR, fun = distHaversine) /1000
rownames(coordsgeo_dist_minusBAR) <- coord_minusBAR$Site_1_abrev
colnames(coordsgeo_dist_minusBAR) <- coord_minusBAR$Site_1_abrev

coordsgeo_dist_minusBAR[upper.tri(coordsgeo_dist_minusBAR)] <- t(coordsgeo_dist_minusBAR)[upper.tri(coordsgeo_dist_minusBAR)]
log_plus_geodist <- log10(1+coordsgeo_dist_minusBAR)
geo_sorted_rows <- log_plus_geodist[order(rownames(log_plus_geodist)),]
log_plus_geodist <- geo_sorted_rows[,order(colnames(geo_sorted_rows))]

#checking with PhiPT Genalex values

PhiPT_Genalex <- read_excel("PhiPT.xlsx")
PhiPT_Genalex <- PhiPT_Genalex %>%
  column_to_rownames(var="...1")
PhiPT_Genalex <- as.dist(PhiPT_Genalex)

#recreating PhiPT from Genalex versus geographical distances from R
mantel_result_phipt <- mantel(PhiPT_Genalex,log_plus_geodist,permutations=999) #looks almost like in Genalex, okay!
mantel_result_phipt
mantel_r <- round(mantel_result_phipt$statistic, 3)  # Mantel R statistic
mantel_p <- mantel_result_phipt$signif # p-value


# Extract lower triangles of the distance matrices for comparison
# Extract the lower triangles
lower_triangle_geo_minusBAR <- log_plus_geodist[lower.tri(log_plus_geodist)]
lower_triangle_geo_minusBAR #geographic coordinates without BAR, calculated in R, but correspond to Martina's values

# Combine into a data frame
cc_distances_df_logplus1 <- data.frame(
  Geographic_Distance = lower_triangle_geo_minusBAR,
  Genetic_Distance = PhiPT_Genalex
)

# Plot the relationship
ggplot(cc_distances_df_logplus1, aes(x = Geographic_Distance, y = Genetic_Distance)) +
  geom_point(alpha = 0.6) +
  geom_smooth(method = "lm", color = "blue") +
  labs(title = "Isolation by Distance",
       x = "log10(1+Geographic Distance)",
       y = "Genetic Distance PhiPT") +
  theme_publish() +
  annotate("text", 
           x = Inf, y = Inf, 
...
#creating genetic distance matrix with dist.genpop and method Nei =1
setPop(myData_genind_allMarkersOnly_allButBAR) <- ~Pop
cc_myData_genind_allMarkersOnly_allButBAR <- clonecorrect(myData_genind_allMarkersOnly_allButBAR, ~Pop)
cc_myData_genpop_aMO_minusBar <- genind2genpop(cc_myData_genind_allMarkersOnly_allButBAR)
cc_genetic_dist_minusBAR <- dist.genpop(cc_myData_genpop_aMO_minusBar, method="1")
cc_genetic_dist_minusBAR <- as.matrix(cc_genetic_dist_minusBAR)
mat_sorted_rows <- cc_genetic_dist_minusBAR[order(rownames(cc_genetic_dist_minusBAR)),] #ordering rows alphabetically in order to match the other matrices
cc_genetic_dist_minusBAR <- mat_sorted_rows[,order(colnames(mat_sorted_rows))] #ordering column names alphabetically

mantel(cc_genetic_dist_minusBAR, PhiPT_Genalex,permutations=9999) # correlates beautifully

#plotting it:
# Create a logical mask for the lower triangle (including diagonal)
lower_triangle <- cc_genetic_dist_minusBAR[lower.tri(cc_genetic_dist_minusBAR, diag = TRUE)]
# Create a matrix to hold only the lower triangle values (or NA for others)
mat_lower <- cc_genetic_dist_minusBAR
mat_lower[!lower.tri(mat_lower, diag = TRUE)] <- NA  # Replace upper triangle with NA

vec_PhiGenalex <- as.vector(PhiPT_Genalex)
cc_genetic_dist_minusBAR_lower <- cc_genetic_dist_minusBAR[lower.tri(cc_genetic_dist_minusBAR)]
vec_neiGenetic <- as.vector(cc_genetic_dist_minusBAR_lower)
cor_test <- cor.test(vec_PhiGenalex, vec_neiGenetic)

# Extract the correlation coefficient and p-value
correlation <- cor_test$estimate
p_value <- cor_test$p.value

plot(vec_PhiGenalex,vec_neiGenetic)
text(x = max(vec_PhiGenalex) * 0.7, y = max(vec_neiGenetic) * 0.9, 
     labels = paste("Correlation = ", round(correlation, 2), 
                    "\nP-value = ", format(p_value, digits = 3)),
     col = "red", cex = 1.2)

#now calculate mantel test for Nei genetic distance

mantel(cc_genetic_dist_minusBAR,log_plus_geodist,permutations=999) #not such a clear result as with PhiPT values
cc_genetic_dist_minusBAR
cc_distances_df_nei <- data.frame(
  Geographic_Distance = lower_triangle_geo_minusBAR,
  Genetic_Distance = cc_genetic_dist_minusBAR_lower
)

ggplot(cc_distances_df_nei, aes(x = Geographic_Distance, y = Genetic_Distance)) +
  geom_point(alpha = 0.6) +
  geom_smooth(method = "lm", color = "blue") +
  labs(title = "Isolation by Distance",
       x = "log10(1+Geographic Distance)",
       y = "Genetic Distance Nei R") +
  theme_minimal()

#------------------------------------------------------------------------------------
# working the same with FST values on cc dataset

cc_truffles.matFst_minusBar <- mat_pw_fst(cc_myData_genind_allMarkersOnly_allButBAR)
cc_truffles.matFst_minusBar <- as.matrix(cc_truffles.matFst_minusBar)
matFst_sorted_rows <- cc_truffles.matFst_minusBar[order(rownames(cc_truffles.matFst_minusBar)),] #ordering rows alphabetically in order to match the other matrices
cc_truffles.matFst_minusBar <- matFst_sorted_rows[,order(colnames(matFst_sorted_rows))] #ordering column names alphabetically

#correlation between Fst and PhiPT values
mantel(cc_truffles.matFst_minusBar,PhiPT_Genalex,permutations=9999) # okay

cc_matFst_lower <- cc_truffles.matFst_minusBar[lower.tri(cc_truffles.matFst_minusBar)]
vec_Fst <- as.vector(cc_matFst_lower)
cor_test_Fst <- cor.test(vec_PhiGenalex, vec_Fst)

# Extract the correlation coefficient and p-value
correlation_Fst <- cor_test_Fst$estimate
p_value_Fst <- cor_test_Fst$p.value

plot(vec_PhiGenalex,vec_Fst)
text(x = max(vec_PhiGenalex) * 0.7, y = max(vec_Fst) * 0.9, 
     labels = paste("Correlation = ", round(correlation_Fst, 2), 
                    "\nP-value = ", format(p_value_Fst, digits = 3)),
     col = "red", cex = 1.2)

#correlation between Nei distance and FST
mantel(cc_genetic_dist_minusBAR,cc_truffles.matFst_minusBar,permutations=9999) #quite high!
cor_test_Fst_Nei <- cor.test(vec_neiGenetic, vec_Fst)

# Extract the correlation coefficient and p-value
correlation_Fst_Nei <- cor_test_Fst_Nei$estimate
p_value_Fst_Nei <- cor_test_Fst_Nei$p.value

plot(vec_neiGenetic,vec_Fst)
text(x = max(vec_neiGenetic) * 0.7, y = max(vec_Fst) * 0.9, 
     labels = paste("Correlation = ", round(correlation_Fst_Nei, 2), 
                    "\nP-value = ", format(p_value_Fst_Nei, digits = 3)),
     col = "red", cex = 1.2)

#missing piece: 
# IBD with FST values

mantel(cc_truffles.matFst_minusBar,log_plus_geodist,permutations=999) #not such a clear result as with PhiPT values

cc_distances_df_Fst <- data.frame(
  Geographic_Distance = lower_triangle_geo_minusBAR,
  Genetic_Distance = cc_matFst_lower
)

ggplot(cc_distances_df_Fst, aes(x = Geographic_Distance, y = Genetic_Distance)) +
  geom_point(alpha = 0.6) +
  geom_smooth(method = "lm", color = "blue") +
  labs(title = "Isolation by Distance",
       x = "log10(1+Geographic Distance)",
...
# Calculate genetic distance using the genpop object
genetic_dist_minus3 <- dist.genpop(myData_genpop_aMO_k6_minus3, method = "1")  # You can choose other methods if needed
genetic_dist_minus3
# try with Laura Schürz's call (euclidean distance and Nei's genetic distance)
mantel(genetic_dist,coordsgeo_dist,method='pearson', permutations=9999)
...
distances_df_minus3 <- data.frame(
  Geographic_Distance = log(lower_triangle_geo),
  Genetic_Distance = lower_triangle_genetic_minus3
)
...
#------------------------------------------------------------------
# try it with genetic distance coming from Spagedi Analysis

mantel(GenDist_spagedi,coordsgeo_dist,method='pearson',permutations=9999)

lower_triangle_genetic_spagedi <- GenDist_spagedi[lower.tri(GenDist_spagedi)]
distances_df_spagedi <- data.frame(
  Geographic_Distance = log(lower_triangle_geo),
  Genetic_Distance_spagedi = lower_triangle_genetic_spagedi
)

# Plot the relationship
ggplot(distances_df_spagedi, aes(x = Geographic_Distance, y = Genetic_Distance_spagedi)) +
  geom_point(alpha = 0.6) +
  geom_smooth(method = "lm", color = "blue") +
  labs(title = "Isolation by Distance (Spagedi)",
       x = "log() Geographic Distance",
...
#----------------------------------------------------------------
#try also the RST matrix

mantel(GenDist_RST_spagedi,coordsgeo_dist,method='pearson',permutations=9999)
mantel(GenDist_RST_spagedi,coordsgeo_dist,method='spearman', permutations=9999)

lower_triangle_genetic_rst_spagedi <- GenDist_RST_spagedi[lower.tri(GenDist_RST_spagedi)]
distances_df_RST_spagedi <- data.frame(
  Geographic_Distance = log(lower_triangle_geo),
  Genetic_Distance_spagedi = lower_triangle_genetic_rst_spagedi
)

# Plot the relationship
ggplot(distances_df_RST_spagedi, aes(x = Geographic_Distance, y = Genetic_Distance_spagedi)) +
  geom_point(alpha = 0.6) +
  geom_smooth(method = "lm", color = "blue") +
  labs(title = "Isolation by Distance (Spagedi)",
       x = "log() Geographic Distance",
       y = "Genetic Distance Rst/(1-Rst)") +
  theme_minimal()
...
locus_table_aMO_SY <- data.frame(t(
  sapply(seppop(myData_genind_allMarkersOnly),
    function(ls) poppr::locus_table(ls,lev="allele"))))
write.csv(locus_table_aMO_SY,"check locustable.csv")
summary(myData_genind_allMarkersOnly)
...
locus_table_aMO_hexp <- locus_table_aMO_SY %>%
  ...
  separate(.,Pop,sep="_", into=c("Pop","SamplingYear"))
#write.csv(locus_table_aMO_hexp,"locus_table_aMO_hexp.csv")
...
n_samples_allMarkersOnly_SY <- n_samples_allMarkersOnly_SY %>%
  select(Pop,SamplingYear,N) %>%
  pivot_wider(names_from=SamplingYear, values_from=N) %>%
  select(.,-"NA") %>%
  replace(is.na(.),0)
#write.csv(n_samples_allMarkersOnly_SY,"n_samples_allMarkersOnly_SY.csv")


#-------------------------------------------------------------------------
# remove boundary effects: definition: less than  half of the sample size of neighboring year's sample size (only outside)
setPop(myData_genind_allMarkersOnly) <- ~Pop/SamplingYear
myData_genind_allMarkersOnly_Boundaries <- popsub(myData_genind_allMarkersOnly,exclude=c("ALD_2014","ALD_2015","BOB_2011,FRB_2014","FRI_2018","GEN_2018","HAN_2016","HAN_2019","KON_2013","NEU_2021","RIE_2015","SCD_2017","SCG_2013","SCG_2014","SCS_2017","UEB_2019","WSL_2012","WSL_2021","BAR_2014"))

locus_table_aMO_SY_noEE <- data.frame(t(
  sapply(seppop(myData_genind_allMarkersOnly_Boundaries),
    function(ls) poppr::locus_table(ls,lev="allele"))))
#write.csv(locus_table_aMO_SY,"check locustable.csv")

# choose only the columns corresponding to Hexp values and rename them with their loci

locus_table_aMO_hexp_noEE <- locus_table_aMO_SY_noEE %>%
  select(X31:X45) %>%
  dplyr::rename(aest06_1 = X31,aest07_1=X32, aest15_1=X33, aest26_1=X34,aest28_1=X35, aest35_1=X36, aest36_1=X37, aest01_1=X38, aest10_1=X39, aest18_1=X40, aest24_1=X41, aest25_1=X42, aest29_1=X43, aest31_1=X44, mean=X45) %>%
  tibble::rownames_to_column("Pop") %>%
  separate(.,Pop,sep="_", into=c("Pop","SamplingYear"))
...
locus_table_SY_hexp_noEE_pivot <- pivot_longer(locus_table_aMO_hexp_noEE,cols=3:16,names_to="locus",values_to="value")

# Sample size list per population
sample_sizes_noEE <- c("ALD" = 52, "BOB" = 423, "BOH" = 40, "BRU"=178, "BUR" = 223, "FRB"=12, "FRE" = 214, "FRI"=117, "GEN"=9,"HAN"=61, "KON"=209,"LIM"=216, "NEU"=30,"RIE"=58, "SCD"=97,  "SCG"=12,"SCL"=99,"SCS"=41,"TRO"=34,"UEB"=150,"UST"=35,"WSL"=278)

# Modify facet labels to include sample sizes
facet_labels_noEE <- paste(names(sample_sizes_noEE), "(n =", sample_sizes_noEE,")")
names(facet_labels_noEE) <- names(sample_sizes_noEE)

#emf(file= "allMarkersOnly_Pop_SY_hexp_nEE.emf") # Opens a device
ggplot(locus_table_SY_hexp_noEE_pivot,aes(x=as.numeric(SamplingYear), y=value)) +
  geom_point(pch=20, alpha=0.3, size=0.8) +
  geom_smooth(method="gam", formula=y~s(x,k=2), colour="darkgreen", linewidth=0.6) +
  facet_wrap(vars(Pop), labeller = labeller(Pop = facet_labels_noEE)) +
  labs(y="Nei's genetic diversity Hexp", x="Sampling year") +
  ggtitle("Nei's genetic diversity per locus, Sampling year and Population", subtitle="Edge effects removed") +
  theme(aspect.ratio=0.4,
    strip.background = element_blank(),
    strip.text=element_text(size=7,hjust=0.1, vjust=0.5),
    panel.grid = element_blank(),
...
#write.csv(cc_locus_table_aMO_SY,"check locustable cc.csv")
...
#write.csv(cc_locus_table_aMO_hexp,"cc_locus_table_aMO_hexp.csv")
...
cc_n_samples_allMarkersOnly_SY <- cc_n_samples_allMarkersOnly_SY %>%
  select(Pop,SamplingYear,N) %>%
  pivot_wider(names_from=SamplingYear, values_from=N) %>%
  select(.,-"NA") %>%
  replace(is.na(.),0)
#write.csv(cc_n_samples_allMarkersOnly_SY,"cc_n_samples_allMarkersOnly_SY.csv")

#there's almost nothing to look at anymore on the level individual / MLG. Only look at two populations with big sample size / many MLGS

# remove boundary effects: definition: less than  half of the sample size of neighboring year's sample size (only outside)
setPop(cc_myData_genind_allMarkersOnly) <- ~Pop
cc_myData_genind_allMarkersOnly_Boundaries <- popsub(cc_myData_genind_allMarkersOnly,c("BOB","KON"))
                                                     
setPop(cc_myData_genind_allMarkersOnly_Boundaries) <- ~Pop/SamplingYear
cc_locus_table_aMO_SY_noEE <- data.frame(t(
  sapply(seppop(cc_myData_genind_allMarkersOnly_Boundaries),
    function(ls) poppr::locus_table(ls,lev="allele"))))
#write.csv(locus_table_aMO_SY,"check locustable.csv")

# choose only the columns corresponding to Hexp values and rename them with their loci

cc_locus_table_aMO_hexp_noEE <- cc_locus_table_aMO_SY_noEE %>%
  select(X31:X45) %>%
  dplyr::rename(aest06_1 = X31,aest07_1=X32, aest15_1=X33, aest26_1=X34,aest28_1=X35, aest35_1=X36, aest36_1=X37, aest01_1=X38, aest10_1=X39, aest18_1=X40, aest24_1=X41, aest25_1=X42, aest29_1=X43, aest31_1=X44, mean=X45) %>%
  tibble::rownames_to_column("Pop") %>%
  separate(.,Pop,sep="_", into=c("Pop","SamplingYear"))
...
cc_locus_table_SY_hexp_noEE_pivot <- pivot_longer(cc_locus_table_aMO_hexp_noEE,cols=3:16,names_to="locus",values_to="value")

# Sample size list per population
cc_sample_sizes_noEE <- c("BOB" = 115, "KON"=25)

# Modify facet labels to include sample sizes
cc_facet_labels_noEE <- paste(names(cc_sample_sizes_noEE), "(n =", cc_sample_sizes_noEE,")")
names(cc_facet_labels_noEE) <- names(cc_sample_sizes_noEE)

#emf(file= "cc_allMarkersOnly_Pop_SY_hexp_nEE.emf") # Opens a device
ggplot(cc_locus_table_SY_hexp_noEE_pivot,aes(x=as.numeric(SamplingYear), y=value)) +
  geom_point(pch=20, alpha=0.3, size=0.8) +
  geom_smooth(method="gam", formula=y~s(x,k=2), colour="darkgreen", linewidth=0.6) +
  facet_wrap(vars(Pop), labeller = labeller(Pop = cc_facet_labels_noEE)) +
  labs(y="Nei's genetic diversity Hexp", x="Sampling year") +
  ggtitle("Nei's genetic diversity per locus, Sampling year and Population", subtitle="Clone corrected dataset") +
  theme(aspect.ratio=0.4,
    strip.background = element_blank(),
    strip.text=element_text(size=7,hjust=0.1, vjust=0.5),
    panel.grid = element_blank(),
...
```{r persistence}
crosspop <- T_all_withMLGs_allMarkersOnly %>%
  select(MLG,Site_1_abrev) %>%
  group_by(MLG) %>%
  filter(n_distinct(Site_1_abrev) > 1) %>% # Filter MLGs appearing in more than one population
  summarize(Populations = toString(unique(Site_1_abrev))) # List all populations for each MLG

write.csv(crosspop,"crosspop.csv")
